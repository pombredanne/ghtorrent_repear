...
def run(project_id, repo_path, cursor, **options):
    ...
    for repos in os.listdir():
        if(repos == repoName):
            ...
            release_score = 0
            if(len(stream) > 3 ):
                if(len(stream)-2 > 0 and len(stream)-3 > 0):
                    time1 = stream[len(stream)-2].split("| ")[1].split(" ")
                    Y1 = int(time1[4])
                    M1 = int(strptime(time1[1],'%b').tm_mon)
                    D1 = int(time1[2])
                    end = datetime(Y1,M1,D1)
                    time2 = stream[len(stream)-3].split("| ")[1].split(" ")
                    Y2 = int(time2[4])
                    M2 = int(strptime(time2[1],'%b').tm_mon)
                    D2 = int(time2[2])            
                    start = datetime(Y2,M2,D2)
                    print("Y1,M1,D1: ",Y1,M1,D1)
                    print("Y2,M2,D2: ",Y2,M2,D2)
                    numberOfMonths = 0
